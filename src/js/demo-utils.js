...
function cleanToolLanguageForDisplay(value = '') {
  return String(value || '')
    ...
    .replace(/\bSplit Regions\s*:/gi, 'Split regions:')
    .replace(/\bSplit regions\s*:\s*A\s*,\s*B\s*,\s*C\s*,\s*D\s*,\s*E\s*\./gi, 'Split different regions.')
    .replace(/\bForeman-\[A-E\]/gi, 'Foremen')
    .replace(/\bRegion\s+\[A-E\]/gi, 'different regions')
    .replace(/\bfive regions\b/gi, 'different regions')
    .replace(/\bsubtasks\s*\[(?:target|different)\s+regions\]/gi, 'subtasks')
    .replace(/\bRegions?\s+A-E\b/gi, 'regions')
    .replace(/\bRegion-[A-E]\.json\b/gi, 'region.json')
    .replace(/\bRegion-[A-E]\b/gi, 'region')
    .replace(/\b[A-E]-\d{2}\s+through\s+[A-E]-\d{2}(?:\s+plus\s+[A-E]-\d{2}\s+through\s+[A-E]-\d{2})?\b/gi, 'placement subplans')
    .replace(/\btrace collected\s+\d+\s*\/\s*\d+\b/gi, 'trace collected')
    .replace(/\bfor\s+placement subplans\b/gi, 'for placement subplans')
